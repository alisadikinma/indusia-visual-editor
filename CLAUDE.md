...
| Status | M0 ✓ · M1 ✓ · M2 Phase 2.1 ✓ · 2.2 ✓ · 2.2b ✓ · 2.2c ✓ defect→detector mapping + derive_inspect_scope; next: Phase 2.3 frontend BomTable + Wizard step 1 |
...
| Tests | 67 tests pass: health, db_connection, 3× db_models, 7× projects CRUD, 7× assets, 7× bom_parser, 6× bom_upload (persist, link, 422 on bad bom + no orphan rows, REPLACE on re-upload, dedup no-op, classifier hints persisted), 26× mi_classifier, 7× derive_inspect_scope (single region, skipped, multi-region, unknown criterion, solder_short whole-side, all-9-criteria-mapped, empty), 2× graphflow spike. `tests/conftest.py` clears `get_engine` lru_cache per-test. | everything else |
