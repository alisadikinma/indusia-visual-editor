...
| Status | M0 ✓ · M1 ✓ · M2 ✓ · M3 ✓ · M4 ✓ · M5 ✓ · M6 ✓ · M7 ✓ · M8 ✓ (8.1 `GET /api/projects/{id}/dataset/stats?side=` — `compute_dataset_stats` helper walks latest Label via `derive_inspect_scope` + joins `bom_items` for MI/SMT split, 8.2 `services/llm/hyperparams.suggest_hyperparams` + `prompts/hyperparams.md` — pydantic-bounded `Hyperparameters{epochs ∈ [5,200], batch_size ∈ [4,64], augmentation_intensity ∈ low/medium/high, notes}`, 8.3 `POST /api/projects/{id}/training/suggest-hyperparams` composer route + `Gate1View.vue` at `/projects/:id/gate1` — stats grid + per-criterion grid + Gemma suggestion card + "Mulai Training" button disabled when no labels (404), 8.4 `TrainingProgressView.vue` at `/projects/:id/training/:runId` — `EventSource(streamProgressUrl)` consumer with running/epoch/succeeded/failed/cancelled state badge, history list, clean unmount); next: M9 eval view (predictions fetch + EvalView with per-component F1 + worst-FP/FN grid + prev-run comparison) |
...
Only commit history is authoritative — never invent state. As of 2026-05-23 (M8 close):

| Layer | Built | Not yet built |
|---|---|---|
| Backend | FastAPI app, `GET /health`, `/api/projects` CRUD, `/api/projects/{id}/assets` upload + list + binary, `/api/projects/{id}/bom_items` GET list, `/api/projects/{id}/llm/plan` POST + GET, `/api/projects/{id}/llm/prelabel` POST + GET (latest-wins per side), `/api/projects/{id}/adapt` POST + GET history, `/api/projects/{id}/labels/task?side=` GET, `/api/projects/{id}/labels?side=` POST + GET, `/api/projects/{id}/training/start` POST, `/api/projects/{id}/training` GET, `/api/training/{run_id}/stream` GET (SSE relay), **`/api/projects/{id}/dataset/stats?side=` GET — Gate-1 dataset stats (latest Label via `derive_inspect_scope` + bom_items MI/SMT join; 404 when no label)**, **`/api/projects/{id}/training/suggest-hyperparams?side=` POST — Gate-1 composer: stats + Gemma 4 hyperparams in one round-trip (404 no-label, 502 on Ollama failure)**, DB models (Project / Asset / BomItem / ProposedPipelineRow / AdaptRun / PreLabel / Label / TrainRun), `get_session` async dep, exception handlers for 404/409/413/422/502 + generic HTTPException all return `{status, message, data}` shape, fs storage at `IVE_STORAGE_ROOT` with SHA256 dedup, **Ollama async client** (`services/llm/client.py`), **planner** (`services/llm/planner.py`), **prelabel orchestrator** (`services/llm/prelabel.py`), **`TrainingClient`** (`services/inspect_service/training_client.py`), **`compute_dataset_stats`** (`routes/dataset_stats.py` helper exported for reuse), **`suggest_hyperparams`** (`services/llm/hyperparams.py` — pydantic-bounded `Hyperparameters{epochs ∈ [5,200], batch_size ∈ [4,64], augmentation_intensity ∈ low/medium/high, notes}`, structured-output via `format=`, `LlmValidationError` on out-of-range/invalid-intensity/non-JSON), **BOM parser**, **MI/SMT heuristic classifier**, **inspect_scope deriver**, **graphflow adapter** | M9 eval, M10 promote + ais subprocess, M11 edge webhooks, M12 chat advisor, M13 auth, M14 Traefik / structured logging / OTel |
| Tests | 233 backend pass + 28 frontend pass = 261 total. New M8 backend tests: **4× dataset_stats route (happy + 404-no-label + multi-side-independence + 422-bad-side)**, **4× hyperparams orchestrator (valid + out-of-range epochs + invalid intensity + non-JSON)**, **3× suggest-hyperparams route (happy + 404 + 422-bad-side)**. New M8 frontend tests: **3× Gate1View (renders stats+suggestion+button + disabled-on-404 + click-starts-training-and-navigates)**, **3× TrainingProgressView (EventSource opens + updates DOM through running→epoch→succeeded + terminal-failed renders error + unmount closes stream)**. 3 opt-in Ollama integration tests + 1 opt-in inspect_service integration test skip when env vars unset. | everything else |
| Frontend | Vue 3 + Vite 5 + TS at `web/`. Routes: `/` Dashboard, `/projects/:id/wizard`, `/projects/:id/labeling`, **`/projects/:id/gate1` (Gate1View — fetches `suggestHyperparams` on mount, renders stats grid + per-criterion grid + Gemma hyperparam card; "Mulai Training" button disabled when 404/no-labels; click → `startTraining` → push to training-progress route)**, **`/projects/:id/training/:runId` (TrainingProgressView — opens `EventSource(streamProgressUrl(runId))`, tracks status badge + currentEpoch + lastLoss + lastMAP + history list; closes on terminal succeeded/failed/cancelled/error; cleanup on unmount)**. Pinia stores: `useProjectsStore`, `useWizardStore`, `useLabelsStore`. Axios clients: `api/projects.ts`, `api/bom.ts`, `api/prelabel.ts`, `api/labels.ts`, **`api/dataset_stats.ts` (`getDatasetStats`)**, **`api/training.ts` (`suggestHyperparams`, `startTraining`, `streamProgressUrl`)**. Components: `StatusBadge`, `BomTable`, `PreLabelPanel`, `LSFEmbed.vue`. CORS allowed on backend for :5173. | LSF runtime smoke verification (manual), skipped-region dim overlay runtime binding |
...
1. **Do not invent hooks, components, or routes.** Check the actual file tree first. As of 2026-05-23 (M8 close) the live routes are: `/health`, `/api/projects` CRUD, `/api/projects/{id}/assets` (POST + GET list + binary), `/api/projects/{id}/bom_items` GET, `/api/projects/{id}/llm/plan` POST + GET, `/api/projects/{id}/llm/prelabel` POST + GET, `/api/projects/{id}/adapt` POST + GET, `/api/projects/{id}/labels/task` GET, `/api/projects/{id}/labels` POST + GET, `/api/projects/{id}/training/start` POST, `/api/projects/{id}/training/suggest-hyperparams` POST, `/api/projects/{id}/training` GET, `/api/training/{run_id}/stream` GET (SSE), `/api/projects/{id}/dataset/stats` GET.
2. **Tables live as of M7 close**: `projects`, `assets`, `bom_items` (with `scope_mode` + `detector_presets` from migration 0006), `proposed_pipelines`, `adapt_runs`, `pre_labels`, `labels`, `train_runs` — via migrations 0001–0007. `deployments`, `edges`, `chat_sessions`, `users`, `organizations` remain design-only — check `alembic/versions/` before assuming a table exists.
