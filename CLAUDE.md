...
| Status | M0 ✓ · M1 ✓ · M2 ✓ · M3 ✓ · M4 ✓ · M5 ✓ · M6 ✓ · M7 ✓ · M8 ✓ · M9 ✓ (9.1 `TrainingClient.get_predictions(job_id)` — GET `/api/eval/{job_id}/predictions` against auto-inspect-service, wraps connection/timeout/non-2xx/non-list errors into `InspectServiceError` subclasses; `GET /api/training/{run_id}/eval` route — returns `{run_id, metrics, predictions, prev_metrics}`, 404 unknown-run, 422 not-succeeded, 502 on service failure, `prev_metrics` = second-most-recent succeeded `TrainRun` for same project (per-project isolation), 9.2 `EvalView.vue` at `/projects/:id/eval/:runId` — mAP card + delta indicator vs prev-run + `MetricChart.vue` per-component F1 (sorted asc, color-coded green/primary/amber/danger thresholds, prev-run F1 outline overlay) + `PredictionGrid.vue` (top-10 worst FP sorted by score desc + top-10 worst FN sorted by score asc, danger/warning border + Bahasa Indonesia copy)); next: M10 Gate 2 + promote-to-prod (`ais model push` subprocess + `deployments` table + Gate2View) |
...
Only commit history is authoritative — never invent state. As of 2026-05-23 (M9 close):

| Layer | Built | Not yet built |
|---|---|---|
| Backend | FastAPI app, `GET /health`, `/api/projects` CRUD, `/api/projects/{id}/assets` upload + list + binary, `/api/projects/{id}/bom_items` GET list, `/api/projects/{id}/llm/plan` POST + GET, `/api/projects/{id}/llm/prelabel` POST + GET (latest-wins per side), `/api/projects/{id}/adapt` POST + GET history, `/api/projects/{id}/labels/task?side=` GET, `/api/projects/{id}/labels?side=` POST + GET, `/api/projects/{id}/training/start` POST, `/api/projects/{id}/training` GET, `/api/training/{run_id}/stream` GET (SSE relay), `/api/projects/{id}/dataset/stats?side=` GET, `/api/projects/{id}/training/suggest-hyperparams?side=` POST, **`/api/training/{run_id}/eval` GET — M9 eval surface: assembles `train_runs.metrics_json` + live `TrainingClient.get_predictions(job_id)` fetch + `prev_metrics` from second-most-recent succeeded TrainRun in same project; 404 unknown-run, 422 if status≠succeeded, 502 on InspectServiceError**, DB models (Project / Asset / BomItem / ProposedPipelineRow / AdaptRun / PreLabel / Label / TrainRun), `get_session` async dep, exception handlers for 404/409/413/422/502 + generic HTTPException all return `{status, message, data}` shape, fs storage at `IVE_STORAGE_ROOT` with SHA256 dedup, Ollama async client, planner, prelabel orchestrator, **`TrainingClient`** (now with `start_training` + `stream_progress` + **`get_predictions`** — GETs `/api/eval/{job_id}/predictions` on auto-inspect-service, wraps connection/timeout/non-2xx/non-list errors into typed `InspectServiceError` subclasses), `compute_dataset_stats`, `suggest_hyperparams`, BOM parser, MI/SMT heuristic classifier, inspect_scope deriver, graphflow adapter | M10 promote + ais subprocess, M11 edge webhooks, M12 chat advisor, M13 auth, M14 Traefik / structured logging / OTel |
| Tests | 244 backend pass + 32 frontend pass = 276 total. New M9 backend tests: **4× `TrainingClient.get_predictions` (happy + connection-error + non-2xx + non-list-body)**, **7× `/api/training/{run_id}/eval` route (happy + prev-metrics-delta + 404-missing-run + 422-not-succeeded + 502-connection-error + 502-bad-response + cross-project-isolation)**. New M9 frontend tests: **4× EvalView (renders metrics+chart+grid + renders prev-run delta indicator + handles null prev_metrics with empty-state hint + 422 error envelope)**. 3 opt-in Ollama integration tests + 1 opt-in inspect_service integration test skip when env vars unset. | everything else |
| Frontend | Vue 3 + Vite 5 + TS at `web/`. Routes: `/` Dashboard, `/projects/:id/wizard`, `/projects/:id/labeling`, `/projects/:id/gate1`, `/projects/:id/training/:runId`, **`/projects/:id/eval/:runId` (EvalView — fetches `getEval(runId)` on mount, renders mAP card + delta indicator (`▲` green / `▼` warning / `■` neutral) vs `prev_metrics?.mAP` + empty-state hint when `prev_metrics === null` + `MetricChart` per-component F1 + `PredictionGrid` top-10 worst FP/FN; 422 error envelope rendered when run not succeeded yet)**. Pinia stores: `useProjectsStore`, `useWizardStore`, `useLabelsStore`. Axios clients: `api/projects.ts`, `api/bom.ts`, `api/prelabel.ts`, `api/labels.ts`, `api/dataset_stats.ts`, `api/training.ts`, **`api/eval.ts` (`getEval(runId)` + `EvalPrediction`/`EvalMetrics`/`EvalResult` types)**. Components: `StatusBadge`, `BomTable`, `PreLabelPanel`, `LSFEmbed.vue`, **`MetricChart.vue` (plain-SVG per-component F1 bars, sorted asc, threshold-coded colors, prev-run F1 outline overlay)**, **`PredictionGrid.vue` (false-positive top-10 sorted by score desc + false-negative top-10 sorted by score asc, danger/warning borders, Bahasa Indonesia explainer)**. CORS allowed on backend for :5173. | LSF runtime smoke verification (manual), skipped-region dim overlay runtime binding |
...
1. **Do not invent hooks, components, or routes.** Check the actual file tree first. As of 2026-05-23 (M9 close) the live routes are: `/health`, `/api/projects` CRUD, `/api/projects/{id}/assets` (POST + GET list + binary), `/api/projects/{id}/bom_items` GET, `/api/projects/{id}/llm/plan` POST + GET, `/api/projects/{id}/llm/prelabel` POST + GET, `/api/projects/{id}/adapt` POST + GET, `/api/projects/{id}/labels/task` GET, `/api/projects/{id}/labels` POST + GET, `/api/projects/{id}/training/start` POST, `/api/projects/{id}/training/suggest-hyperparams` POST, `/api/projects/{id}/training` GET, `/api/training/{run_id}/stream` GET (SSE), `/api/training/{run_id}/eval` GET, `/api/projects/{id}/dataset/stats` GET.
