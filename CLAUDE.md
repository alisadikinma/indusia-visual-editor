...
| Status | M0–M10 ✓ · M11 ✓ (11.1 `edges` table via migration `0009_edges` — UUID PK + UNIQUE name + webhook_url + JSONB `version_policy` server_default `{"mode":"auto_pull_latest"}` + registered_at + last_seen_at; `POST /api/edges` (201 with HttpUrl-validated body, 409 on duplicate name, 422 on bad URL) + `GET /api/edges` list + `PUT /api/edges/{id}` policy update; 11.2 `services/edge/notify.notify_edges` — async fan-out over registered edges with 1/2/4s exponential backoff (3 attempts), wraps httpx connection/timeout/transport errors per-edge into `NotifyOutcome{edge_id, name, ok, attempts, error}` so a single bad edge does NOT roll back the deployment; pinned edges receive their pinned `(model_name, version)` instead of the deployment's version; `_sleep` indirected so tests patch to zero. Wired into `POST /api/projects/{id}/deploy` success path — after `push_model` returns ok, notify fan-out runs and per-edge outcomes land in `deployments.edges_notified` JSONB keyed by edge name. Notify is SKIPPED on push failure (edges must not consume incomplete deployments); 11.3 `PUT /api/edges/{id}/pin` — body `{model_name, version}` switches policy to `{mode:'pinned', model_name, version}`, empty body resets to auto_pull_latest; 422 when only one of the two fields supplied) | next: M12 chat advisor (Ollama streaming + chat_sessions table + Vue drawer) |
...
Only commit history is authoritative — never invent state. As of 2026-05-23 (M11 close):

| Layer | Built | Not yet built |
|---|---|---|
| Backend | FastAPI app, `GET /health`, `/api/projects` CRUD, `/api/projects/{id}/assets`, `/api/projects/{id}/bom_items` GET, `/api/projects/{id}/llm/plan` POST+GET, `/api/projects/{id}/llm/prelabel` POST+GET, `/api/projects/{id}/adapt` POST+GET, `/api/projects/{id}/labels/task` GET, `/api/projects/{id}/labels` POST+GET, `/api/projects/{id}/training/start` POST, `/api/projects/{id}/training` GET, `/api/training/{run_id}/stream` GET (SSE), `/api/training/{run_id}/eval` GET, `/api/projects/{id}/dataset/stats` GET, `/api/projects/{id}/training/suggest-hyperparams` POST, `/api/projects/{id}/deploy` POST+GET, **`/api/edges` POST (201 / 409 dup-name / 422 bad-URL) + GET + `PUT /api/edges/{id}` policy update + `PUT /api/edges/{id}/pin` rollback (or unpin via empty body) — M11 edge registry & manual pin**, DB models (Project / Asset / BomItem / ProposedPipelineRow / AdaptRun / PreLabel / Label / TrainRun / Deployment / **Edge**), `get_session` async dep, exception handlers 404/409/413/422/502 + generic HTTPException return `{status, message, data}` envelope, fs storage at `IVE_STORAGE_ROOT` with SHA256 dedup, Ollama async client, planner, prelabel orchestrator, `TrainingClient` (start_training + stream_progress + get_predictions), `compute_dataset_stats`, `suggest_hyperparams`, `services/deploy/registry.push_model` (3-step add→commit→push subprocess wrapper + `PushResult` + asyncio.wait_for timeout), **`services/edge/notify.notify_edges`** (async fan-out with 1/2/4s exponential backoff over 3 attempts, per-edge `NotifyOutcome` so a single bad edge does NOT roll back the deployment, pinned edges receive their pinned target not the deployment version; wired into deploy route success path — outcomes persist to `Deployment.edges_notified` JSONB keyed by edge name, SKIPPED on push failure), BOM parser, MI/SMT heuristic classifier, inspect_scope deriver, graphflow adapter | M12 chat advisor, M13 auth, M14 Traefik / structured logging / OTel |
| Tests | 278 backend pass + 36 frontend pass = 314 total. New M11 backend tests: **2× `Edge` model (persists with default policy + UNIQUE name)**, **9× `/api/edges` routes (POST 201 + 409 dup + GET list + PUT policy + PUT 404 + POST 422 bad URL + PUT /pin sets pinned + PUT /pin empty unpins + PUT /pin 404)**, **6× `services/edge/notify.notify_edges` (fans out to all edges + retries-then-succeeds + exhausts-retries-returns-failed-outcome + pinned-edge-uses-pinned-target + empty-edges-returns-empty-list + connection-error-wrapped-per-edge)**, **2× deploy route notify integration (persists edges_notified JSONB on success + skips notify on push failure)**. 3 opt-in Ollama + 1 opt-in inspect_service + 1 opt-in ais integration tests skip when env vars unset. | everything else |
...
1. **Do not invent hooks, components, or routes.** Check the actual file tree first. As of 2026-05-23 (M11 close) the live routes are: `/health`, `/api/projects` CRUD, `/api/projects/{id}/assets` (POST + GET list + binary), `/api/projects/{id}/bom_items` GET, `/api/projects/{id}/llm/plan` POST + GET, `/api/projects/{id}/llm/prelabel` POST + GET, `/api/projects/{id}/adapt` POST + GET, `/api/projects/{id}/labels/task` GET, `/api/projects/{id}/labels` POST + GET, `/api/projects/{id}/training/start` POST, `/api/projects/{id}/training/suggest-hyperparams` POST, `/api/projects/{id}/training` GET, `/api/training/{run_id}/stream` GET (SSE), `/api/training/{run_id}/eval` GET, `/api/projects/{id}/dataset/stats` GET, `/api/projects/{id}/deploy` POST + GET, `/api/edges` POST + GET, `/api/edges/{id}` PUT, `/api/edges/{id}/pin` PUT.
2. **Tables live as of M11 close**: `projects`, `assets`, `bom_items` (with `scope_mode` + `detector_presets` from migration 0006), `proposed_pipelines`, `adapt_runs`, `pre_labels`, `labels`, `train_runs`, `deployments`, `edges` — via migrations 0001–0009. `chat_sessions`, `users`, `organizations` remain design-only — check `alembic/versions/` before assuming a table exists.
