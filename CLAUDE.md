...
| Status | M0 ✓ · M1 ✓ · M2 ✓ · M3 ✓ (3.1 Ollama client, 3.2 pydantic schemas, 3.3 planner skeleton + prompt, 3.4 planner route + `proposed_pipelines` table); next: M4 planner adapter → service config.yaml writer (needs gaspol-plan sub-phase breakdown) |
...
| Tests | 105 backend pass: health, db_connection, 3× db_models, 7× projects CRUD, 7× assets, 7× bom_parser, 6× bom_upload, 26× mi_classifier, 7× derive_inspect_scope, 3× bom_list, 10× ollama_client (mock-transport + error wrapping), 13× llm_schemas (ProposedPipeline/PreLabeledRegion/DefectVerdict/ChatTurn + designator regex + bbox range + literal enforcement), 5× planner (validated output, bad-JSON raises, schema-violation raises, format param, BOM context in prompt), 7× llm route (persist + version + 422 no-bom + 422 no-golden + 404 GET + GET-highest + 502 on garbage), 2× graphflow spike. `tests/conftest.py` clears `get_engine` lru_cache per-test. 2 opt-in integration tests (Ollama) skip when `IVE_OLLAMA_INTEGRATION` unset. | everything else |
