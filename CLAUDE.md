...
| Status | M0–M9 ✓ · M10 ✓ (10.1 `docs/specs/ais-model-push.md` — plan-correction spec: actual `ais` CLI is `ais model add <pcb> --all` → `ais model commit -m <msg>` → `ais model push` (no `--version` flag; git+LFS does versioning), opt-in `tests/spike/test_ais_cli_available.py` skips when binary missing, 10.2 `deployments` table via migration `0008_deployments` — `(project_id, train_run_id) CASCADE` + `status CHECK in (pending|succeeded|failed)` + `model_version` + `edges_notified JSONB` (null until M11) + `error_text`, 10.3 `services/deploy/registry.push_model` — async wrapper running the three-step subprocess sequence with overridable factory test seam, `PushResult{ok, stage ∈ add|commit|push|done|timeout, returncode, stdout, stderr}`, short-circuits on first non-zero exit, `asyncio.wait_for` timeout kills hung processes; `POST /api/projects/{id}/deploy` route — looks up latest succeeded TrainRun (404 missing project, 422 no succeeded run), generates `model_version` from `ended_at` + 6-char run-id, persists row with status='succeeded'/'failed' BEFORE 502 envelope (audit trail always survives), uses project `slug` as `pcb_name`; `GET /api/projects/{id}/deploy` history newest-first, 10.4 `Gate2View.vue` at `/projects/:id/eval/:runId/gate2` — eval summary + previous-deployment side-by-side card + danger-colored "Promote ke Production" button with mandatory confirmation modal (CLAUDE.md §11), `api/deploy.ts` (`promoteToProduction`, `getDeployHistory`), tailwind config gains `danger-hover` + `text-on-primary` tokens that were silently no-op'd in Gate1View since M8); next: M11 edge notification + version pin (edges table + webhook with retry + manual rollback PUT) |
...
Only commit history is authoritative — never invent state. As of 2026-05-23 (M10 close):

| Layer | Built | Not yet built |
|---|---|---|
| Backend | FastAPI app, `GET /health`, `/api/projects` CRUD, `/api/projects/{id}/assets` upload + list + binary, `/api/projects/{id}/bom_items` GET list, `/api/projects/{id}/llm/plan` POST + GET, `/api/projects/{id}/llm/prelabel` POST + GET, `/api/projects/{id}/adapt` POST + GET, `/api/projects/{id}/labels/task?side=` GET, `/api/projects/{id}/labels?side=` POST + GET, `/api/projects/{id}/training/start` POST, `/api/projects/{id}/training` GET, `/api/training/{run_id}/stream` GET (SSE relay), `/api/training/{run_id}/eval` GET, `/api/projects/{id}/dataset/stats?side=` GET, `/api/projects/{id}/training/suggest-hyperparams?side=` POST, **`/api/projects/{id}/deploy` POST + GET — M10 promote-to-prod: latest-succeeded TrainRun → `services/deploy/registry.push_model` (3-step subprocess add→commit→push) → `Deployment` row persisted with full audit trail BEFORE any 502 envelope, 422 if no succeeded run, 404 missing project**, DB models (Project / Asset / BomItem / ProposedPipelineRow / AdaptRun / PreLabel / Label / TrainRun / **Deployment**), `get_session` async dep, exception handlers for 404/409/413/422/502 + generic HTTPException all return `{status, message, data}` shape, fs storage at `IVE_STORAGE_ROOT` with SHA256 dedup, Ollama async client, planner, prelabel orchestrator, `TrainingClient` (start_training + stream_progress + get_predictions), `compute_dataset_stats`, `suggest_hyperparams`, **`services/deploy/registry.push_model`** (async wrapper around `ais model {add,commit,push}` with overridable subprocess factory test seam + `PushResult{ok, stage, returncode, stdout, stderr}` + asyncio.wait_for timeout, never raises — always returns PushResult so the route always persists an audit row), BOM parser, MI/SMT heuristic classifier, inspect_scope deriver, graphflow adapter | M11 edge webhooks, M12 chat advisor, M13 auth, M14 Traefik / structured logging / OTel |
| Tests | 259 backend pass + 36 frontend pass = 295 total. New M10 backend tests: **4× `Deployment` model (insert+links + status-CHECK + project-cascade + train-run-cascade)**, **5× `push_model` registry wrapper (add+commit+push order + add-failure short-circuit + push-failure-after-add-commit + custom-ais-binary-path + timeout kills hung process)**, **6× `/api/projects/{id}/deploy` route (happy persists + push-failure persists row + status='failed' + 422-no-succeeded-run + 404-unknown-project + picks-latest-succeeded-run + GET history)**, **1× ais-CLI spike (skips when binary not installed)**. New M10 frontend tests: **4× Gate2View (renders eval + prev-deployment + Promote button + opens-modal-on-click + confirm-triggers-API + cancel-closes-without-call)**. New env vars: `IVE_AIS_BINARY`, `IVE_REGISTRY_ROOT`, `IVE_AIS_PUSH_TIMEOUT_SECS`. 3 opt-in Ollama + 1 opt-in inspect_service + 1 opt-in ais integration tests skip when env vars unset. | everything else |
| Frontend | Vue 3 + Vite 5 + TS at `web/`. Routes: `/` Dashboard, `/projects/:id/wizard`, `/projects/:id/labeling`, `/projects/:id/gate1`, `/projects/:id/training/:runId`, `/projects/:id/eval/:runId`, **`/projects/:id/eval/:runId/gate2` (Gate2View — fetches `getEval(runId)` + `getDeployHistory(projectId)` in parallel on mount, renders current-run mAP + prev-deployment version side-by-side card + danger-colored "Promote ke Production" button with mandatory confirmation modal (Batal / Ya Promote); `promoteToProduction(projectId)` POST → success badge + version label; CLAUDE.md §11 — no auto-approval)**. Pinia stores: `useProjectsStore`, `useWizardStore`, `useLabelsStore`. Axios clients: `api/projects.ts`, `api/bom.ts`, `api/prelabel.ts`, `api/labels.ts`, `api/dataset_stats.ts`, `api/training.ts`, `api/eval.ts`, **`api/deploy.ts` (`promoteToProduction(projectId)` + `getDeployHistory(projectId)` + `Deployment` type)**. Components: `StatusBadge`, `BomTable`, `PreLabelPanel`, `LSFEmbed.vue`, `MetricChart.vue`, `PredictionGrid.vue`. Tailwind tokens gain `danger-hover: #DC2626` + `text-on-primary: #FFFFFF` (fixes silent-no-op of pre-existing M8.3 Gate1View references). CORS allowed on backend for :5173. | LSF runtime smoke verification (manual), skipped-region dim overlay runtime binding |
...
| DB | Alembic migrations 0001–0007 applied: `projects`, `assets`, `bom_items` (with `scope_mode` + `detector_presets` cols from 0006), `proposed_pipelines`, `adapt_runs`, `pre_labels`, `labels`, **`train_runs`** (project_id + adapt_run_id FK CASCADE, service_job_id, status CHECK in pending/running/succeeded/failed/cancelled, metrics_json, started_at, ended_at, error_text) tables live with UUID PKs, TIMESTAMPTZ, JSONB, CHECK-constraint enums. All downgrade→upgrade cycles clean. | Future migrations per phase |
...
1. **Do not invent hooks, components, or routes.** Check the actual file tree first. As of 2026-05-23 (M10 close) the live routes are: `/health`, `/api/projects` CRUD, `/api/projects/{id}/assets` (POST + GET list + binary), `/api/projects/{id}/bom_items` GET, `/api/projects/{id}/llm/plan` POST + GET, `/api/projects/{id}/llm/prelabel` POST + GET, `/api/projects/{id}/adapt` POST + GET, `/api/projects/{id}/labels/task` GET, `/api/projects/{id}/labels` POST + GET, `/api/projects/{id}/training/start` POST, `/api/projects/{id}/training/suggest-hyperparams` POST, `/api/projects/{id}/training` GET, `/api/training/{run_id}/stream` GET (SSE), `/api/training/{run_id}/eval` GET, `/api/projects/{id}/dataset/stats` GET, `/api/projects/{id}/deploy` POST + GET.
2. **Tables live as of M10 close**: `projects`, `assets`, `bom_items` (with `scope_mode` + `detector_presets` from migration 0006), `proposed_pipelines`, `adapt_runs`, `pre_labels`, `labels`, `train_runs`, `deployments` — via migrations 0001–0008. `edges`, `chat_sessions`, `users`, `organizations` remain design-only — check `alembic/versions/` before assuming a table exists.
